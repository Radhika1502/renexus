...
describe('TaskDependencies Component', () => {
  ...
  it('renders dependencies section with correct title', () => {
    render(
      <QueryClientProvider client={queryClient}>
        <TaskDependencies taskId="task-1" />
      </QueryClientProvider>
    );
  ...
  it('shows empty state when there are no dependencies', () => {
    // Mock the useTask hook to return a task with no dependencies for this test
  ...
  it('disables add dependency button when all tasks are already dependencies', () => {
    // Mock the useTask hook to return a task with all dependencies for this test
    require('../../hooks/useTask').useTask.mockImplementationOnce(() => ({
      task: {
        ...mockTask,
